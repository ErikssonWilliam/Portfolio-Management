...
for i = 1:length(t)
    ...
    %VAD SKA OPTIONS VÄRDET VARA????????????????????????
   % IV(i)= Implicit_sigma(10,S(i),K(i),r(i),q(i),T(i),0.00001, 0.2);
end
...
window_size = 500;          % Historical window for simulations
...
index_prices = readmatrix('portfolioManagerV4Projekt3.xlsm', 'Sheet', 'Refinitiv', 'Range', 'AB5:AB758');
...
for day = 1:length(t)
    ...
    start_idx = max(1, day-window_size);
    end_idx = day-1;
    available_returns = log_returns(start_idx:end_idx);
    % Each day, you simulate 500 scenarios of the index (based on past returns and current index.
    random_indices = randi(length(log_returns), noOfSimulations, 1);
    ...
    portfolio_history(day).VaR = VaR_95;
